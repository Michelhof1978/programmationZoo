...
// export default Parc;
Il semble que vous ayez un problème de syntaxe dans votre composant Parc. Voici la version corrigée :
...
Dans cette version corrigée, j'ai modifié quelques points :

- J'ai ajouté la gestion des erreurs avec un `catch` dans la requête Axios.
- J'ai corrigé la syntaxe des méthodes `handleSelectionFamille` et `handleSelectionContinent` pour qu'elles soient bien définies en tant que fonctions.
- J'ai mis à jour les parenthèses autour du retour de la fonction de rendu pour que le code soit correctement interprété par React.

Assurez-vous d'importer correctement les dépendances nécessaires et de vérifier que les chemins des fichiers sont corrects.